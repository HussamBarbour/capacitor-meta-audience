...
public class CapacitorMetaAudiencePlugin: CAPPlugin, FBInterstitialAdDelegate,FBAdViewDelegate {
    ...
    @objc func initialize(_ call: CAPPluginCall) {
        ...
        DispatchQueue.main.async {
            if #available(iOS 14, *) {
                // Requesting user authorization for tracking
                ATTrackingManager.requestTrackingAuthorization { status in
                    switch status {
                    case .authorized:
                        // User has granted permission to track
                        FBAdSettings.setAdvertiserTrackingEnabled(true)
                        call.resolve(["trackingStatus": "authorized"])
                    case .denied:
                        // User has denied permission to track
                        FBAdSettings.setAdvertiserTrackingEnabled(false)
                        call.resolve(["trackingStatus": "denied"])
                    case .restricted, .notDetermined:
                        // User has not made a choice or is restricted from making a choice
                        FBAdSettings.setAdvertiserTrackingEnabled(false)
                        call.resolve(["trackingStatus": "restricted"])
                    @unknown default:
                        // Other unknown status
                        FBAdSettings.setAdvertiserTrackingEnabled(false)
                        call.resolve(["trackingStatus": "unknown"])
                    }
                }
            }
        }
